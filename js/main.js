...
(async () => {
    ...
    if (remoteProgress) {
        if (remoteProgress.xp) localStorage.setItem('laeticia_xp', remoteProgress.xp);
        if (remoteProgress.level) localStorage.setItem('laeticia_level', remoteProgress.level);
        if (remoteProgress.academy_level !== undefined) localStorage.setItem('academy_level', remoteProgress.academy_level);
        if (remoteProgress.completed_lessons) localStorage.setItem('laeticia_completed', JSON.stringify(remoteProgress.completed_lessons));
        if (remoteProgress.grammar_completed) localStorage.setItem('laeticia_grammar', JSON.stringify(remoteProgress.grammar_completed));
        if (remoteProgress.exam_completed) localStorage.setItem('laeticia_exam_completed', JSON.stringify(remoteProgress.exam_completed));
        if (remoteProgress.dad_progress) localStorage.setItem('laeticia_dad_progress', JSON.stringify(remoteProgress.dad_progress));
    ...
    window.syncProgress = async function() {
        const data = {
            xp: parseInt(localStorage.getItem('laeticia_xp') || '0'),
            level: parseInt(localStorage.getItem('laeticia_level') || '1'),
            academy_level: parseInt(localStorage.getItem('academy_level') || '0'),
            completed_lessons: JSON.parse(localStorage.getItem('laeticia_completed') || '[]'),
            grammar_completed: JSON.parse(localStorage.getItem('laeticia_grammar') || '[]'),
            exam_completed: JSON.parse(localStorage.getItem('laeticia_exam_completed') || '[]'),
            dad_progress: JSON.parse(localStorage.getItem('laeticia_dad_progress') || '{}')
        };
